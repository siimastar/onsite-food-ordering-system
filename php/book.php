...
if (isset($_POST['submit'])) {
  ...
  if (
    empty($name) || empty($country) || empty($contact) || empty($email) || empty($hotelname) || empty($category) || empty($price) || empty($bank)
  ) {
    $error_message = "<h5 style=\"color:red; font-weight:lighter\">one or more required fields are blank!!!</h5>";
  } else {

    $query = "INSERT INTO book VALUES(null,'{$name}','{$country}','{$contact}','{$email}','{$hotelname}','{$category}','{$price}','{$bank}')";
...
$id = $_GET['id'];
$sql = "SELECT * FROM dishes WHERE available='yes' AND id='$id'";
...
if ($result->num_rows > 0) {
  ...
  echo "this Dish is not available";
}
    ...
    <div id="logrest">
      <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <table style="auto">
          <tr>
            <td>Name:</td>
            <td><input type="text" name="name" placeholder="name" value="<?php echo $fname; ?>"></td>
            <td>Country:</td>
            <td><input type='text' name='country' placeholder='eg:  Nigeria' value="Nigeria"></td>
          </tr>
          <tr>
            <td>Contact No:</td>
            <td><input type="text" name='contact' placeholder='eg: +2348169895827' value="<?php echo $phone; ?>"></td>
            <td>Email:</td>
            <td><input type='email' name='email' placeholder="Enter the email" value="<?php echo $email; ?>"></td>
          </tr>
          <tr>
            <td>Dish Name:</td>
          ...
          <tr>
            <td colspan='5' align='center'><input type='submit' name='submit' value='Book'></td>
          </tr>
        </table>
      </form>
    </div>
